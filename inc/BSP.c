...
void BSP_Button1_Init(void){
  ...
  //GPIO_PORTD_AMSEL_R &= ~0x40;     // 3) disable analog on PD6, shouldn't need to since no analog option
...
#define BUTTON1   (*((volatile uint32_t *)0x40007100))  /* PD6: 10.4 Register Map  */
...
void static adcinit(void){
  ...
  ADC0_PC_R &= ~0xF;               // 8) clear max sample rate field (891)
  ADC0_PC_R |= 0x1;                //    configure for 125K samples/sec 
  ADC0_SSPRI_R = 0x3210;           // 9) Sequencer 3 is lowest priority (841)
...
// Input: none
// Output: none
void BSP_Accelerometer_Init(void){ // page 17 for configuration
  SYSCTL_RCGCGPIO_R |= 0x00000008; // 1) activate clock for Port D
  while((SYSCTL_PRGPIO_R&0x08) == 0){};// allow time for clock to stabilize
  ...
  ADC0_EMUX_R &= ~0x0F00;          // 11) seq2 is software trigger (continuously sample)
  ADC0_SSMUX2_R = 0x0567;          // 12) set channels for SS2, READ PAGE 801
